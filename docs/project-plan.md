...
_Snapshot as of 2026-05-06. Revisit when GitHub issue counts or triage classifications change._
...
- [Code Triage](#code-triage)
  ...
  - [Project Areas And Current Direction](#project-areas-and-current-direction)
  - [Keep / Adapt / Quarantine / Delete Summary](#keep--adapt--quarantine--delete-summary)
  - [Detailed Migration-Kit Code Triage](#detailed-migration-kit-code-triage)
- [Issue Triage](#issue-triage)
  - [Project-Wide Buckets](#project-wide-buckets)
  - [Immediate Normalization Priorities](#immediate-normalization-priorities)
    - [P0: Prevent Old Migration Direction From Leaking Into Implementation](#p0-prevent-old-migration-direction-from-leaking-into-implementation)
    - [P1: Project-Wide Parent Issues (Canonical)](#p1-project-wide-parent-issues-canonical)
    - [P2: Apply Labels/Project Fields Consistently](#p2-apply-labelsproject-fields-consistently)
  - [Area Notes](#area-notes)
  - [Detailed Migration-Kit Issue Triage](#detailed-migration-kit-issue-triage)
    - [Direct File-Migration Issues](#direct-file-migration-issues)
    - [Migration-Adjacent Issues To Adapt Or Defer](#migration-adjacent-issues-to-adapt-or-defer)
    - [Unsupported Initial Object Families / Features](#unsupported-initial-object-families--features)
    - [Issues That Must Not Gate File-Migration Slice 0](#issues-that-must-not-gate-file-migration-slice-0)
    - [Per-Slice Parent Issues](#per-slice-parent-issues)
    - [Immediate Cleanup Recommendations](#immediate-cleanup-recommendations)
- [Plan](#plan)
  - [Global Rules](#global-rules)
  - [Workstreams](#workstreams)
  - [Recommended Order](#recommended-order)
  - [File-Migration Slice Reference](#file-migration-slice-reference)
  - [Parallel Work Guidance](#parallel-work-guidance)
  - [Required Code Follow-up Issues](#required-code-follow-up-issues)
...
The Keep / Adapt / Quarantine / Delete classifications below describe the disposition of *existing* code under spec precedence. They are not behavioral contracts — specs (per [Source Of Truth](#source-of-truth)) remain authoritative even when a path is classified Keep.

### Package-Level Classification

| Path | Project area | Classification | Why | Primary spec(s) |
| --- | --- | --- | --- | --- |
| `cmd/grizzle` | CLI | Adapt / Quarantine | Current CLI exposes six commands (`snapshot`, `diff`, `migrate`, `status`, `gen`, `sql`); public command shape is not the target. `gen` remains useful; `snapshot`, `diff`, legacy `migrate`, and `status` are not target file-migration commands. The `sql` helper (internal: `runSQL`) is a fresh-database CREATE printer kept separate from the file-migration workflow. See [Quarantine](#quarantine) below for per-command cutover detail. | `kit.md`, `file-migrations-api.md`, `codegen.md` |
...
| `driver/pgx/` | PostgreSQL driver | Adapt | Useful current driver. Still has prepared statement and transaction/test gaps. Migration sessions must not be forced to use this API shape unless it matches the session spec. | `transactions.md`, `query-builder.md`, `file-migrations-api.md` |
...
| `expr/` | SQL expression builders | Adapt | Current query expressions are useful. Some dialect guards and operators remain gaps/bugs. Do not treat query expressions as the file-migration DDL (Data Definition Language) expression model. | `query-builder.md`, `types.md`, `dialects.md` |
...
| `query/` | query builder | Adapt | Core query builder is substantially implemented, but specs list remaining gaps and bugs. Query work must continue from `query-builder.md`, not from file-migration sequence docs. | `query-builder.md`, `relations.md`, `dialects.md` |
...
- For generated columns and standalone sequences, initial file migrations must recognize and reject where specified rather than silently support partial old-model behavior.
...
Current `grizzle gen` is an existing user-facing capability and must be reviewed against `codegen.md`.
...
- Driver transaction gaps must be handled through `transactions.md`.
- File-migration sessions/locking/history must be implemented from `file-migrations-api.md`, `file-migrations-history.md`, and `file-migrations-execution.md`; current query-driver APIs can inform adapters but must not dictate them.
...
This summary is project-wide. The narrower file-migration-scoped tables in [Detailed Migration-Kit Code Triage](#detailed-migration-kit-code-triage) below may classify a package differently when it is *off* the migration-kit path (e.g., `expr/` and `query/` are Adapt project-wide for query/expression parity gaps, but Keep within the file-migration sequence because they are off that path).
...
Current validation status: see [Current State](#current-state) above.
...
| `kit/migrate_mysql.go` | MySQL variant of legacy live-diff migrate/status/history. Includes `MigrateMySQL`, `StatusMySQL`, plus the direct-sync push helpers `PushMySQL`/`DryRunMySQL` (which belong to Follow-up #1's push quarantine). | Same incompatibility as PostgreSQL path. Cleanup must remove the `Status*` helper alongside `Migrate`/`Apply`; push helpers are tracked separately in Follow-up #1. | Same cutover/removal path. | Slice 6, Slice 8 |
| `kit/migrate_sqlite.go` | SQLite variant of legacy live-diff migrate/status/history, including comment-stub skips. Includes `MigrateSQLite`, `StatusSQLite`, plus the direct-sync push helpers `PushSQLite`/`DryRunSQLite` (which belong to Follow-up #1's push quarantine). | Same incompatibility as PostgreSQL path; current unsupported operation handling is especially unsafe as a target precedent. Cleanup must remove the `Status*` helper alongside `Migrate`/`Apply`; push helpers are tracked separately in Follow-up #1. | Same cutover/removal path. | Slice 6, Slice 8 |
...
| `cmd/grizzle sql` (internal: `runSQL`) | Prints fresh-database CREATE SQL from schema definitions. | Useful utility, but not part of the target file-migration workflow. | Keep separate from file-migration public workflow, or defer decision to cleanup. | Slice 8 / deferred |
...
| Slice 5: History, locking, sessions | `driver/*`, `dialect/` (per Keep / Light Adapt table above) | Build new sessions/history/locking; do not adapt old history tables except as negative tests. |
...
| Drivers / transactions / prepared statements | #42, #88, #143, #159, #160, #166, #170, #223, #252, #267, #268 | Work from `transactions.md` and `query-builder.md`; file-migration sessions must reference but not wait on user-facing driver features unless required. |
...
This is the project-wide label scheme. Migration-kit-specific labels (`slice:N`, `spec-required`) appear in [Label Recommendations](#label-recommendations) below.
...
Issues #234, #235, #236, and #259 are near-term correctness/parity items. They must be evaluated against `schema.md`, `types.md`, and `codegen.md`, not delayed just because they are outside the file-migration slice names.

Issues #172, #253, and #254 must be treated carefully: generated columns are recognized in RC.1 snapshots but intentionally unsupported in the initial file-migration target. The first file-migration implementation must add negative validation for generated columns, not partial support through the old `kit.Snapshot` model.
...
Query issues must continue independently against `query-builder.md` and `relations.md`. They are part of the holistic project plan and must not be hidden behind file-migration sequencing.
...
Driver and transaction issues are cross-cutting. They must not block file-migration Slice 0, but Slice 5 migration sessions must consult these issues to avoid duplicating incompatible abstractions.
...
- `Supersede`: issue tracks an old checksum/live-diff/flat-file/meta-direction and must be closed or replaced by a slice-specific issue.
- `Defer`: valid backlog, but outside the initial file-migration implementation sequence.
- `Blocked by spec`: no implementation must start until a dedicated spec or amendment exists.
...
- [#153](https://github.com/sofired/grizzle/issues/153) tracks the right command name (`generate`) but the wrong artifact model (`meta/snapshot.json` plus numbered flat SQL files). It must not become the Slice 4 implementation issue without being rewritten.
...
| [#275](https://github.com/sofired/grizzle/issues/275) | `kit.Migrate: handle semicolons in string literals and PL/pgSQL blocks` | Supersede | Same as #273. This must not drive Slice 6. | `superseded`; future parser work blocked by spec |
| [#280](https://github.com/sofired/grizzle/issues/280) | `return error from splitSQLStatements for unterminated constructs` | Supersede | Same splitter family as #273/#275. The target must not have this helper on the command path. | `superseded`; future parser work blocked by spec |

#### Migration-Adjacent Issues To Adapt Or Defer

These are not all initial file-migration blockers, but they touch schema loading, diffing, introspection, or dialect execution and must be mapped before coding starts.
...
| [#243](https://github.com/sofired/grizzle/issues/243) | default-expression normalization | Defer / adapt | Low-priority unless the strict schema loader or RC.1 fixtures can emit dollar-quoted defaults. If kept, acceptance must reference validated snapshot/default rendering. | Slice 2 / Slice 3 or deferred |
...
| [#259](https://github.com/sofired/grizzle/issues/259) | unsafe/raw numeric defaults | Map | Relevant to strict schema input and literal validation. Acceptance must cite the file-migration literal/default rules rather than only current builder behavior. | Slice 2 |
| [#226](https://github.com/sofired/grizzle/issues/226) | introspection FK action fallback | Rework | Target behavior must fail or diagnose unsupported introspection values; a warning-only fallback is too weak for strict file migrations. | Slice 7, maybe Slice 2 validation fixtures |
| [#225](https://github.com/sofired/grizzle/issues/225) | PostgreSQL FK introspection integration test | Map | Useful for `pull`/introspection validation. Must run under integration-test infrastructure. | Slice 7, testing |
...
| [#160](https://github.com/sofired/grizzle/issues/160) | MySQL/SQLite transaction wrappers | Outside / maybe inform | User-facing transaction wrappers are separate. Slice 5 migration sessions may use `database/sql` directly and must not wait on this. | Not a gate; maybe Slice 5 reference |
...
The ratified file-migration specs intentionally treat these as unsupported in the initial target. Existing issues can remain as backlog, but they must not be implemented into `kit.Snapshot`, `Diff`, SQL generation, or `pull` before the initial RC.1-supported field matrix is in place.
...
| [#254](https://github.com/sofired/grizzle/issues/254) | generated-column introspection/codegen | Defer / blocked by spec | Initial `pull` must detect and reject/report unsupported generated columns according to spec. |
...
| [#250](https://github.com/sofired/grizzle/issues/250) | sequence introspection | Defer / blocked by spec | Initial `pull` must reject unsupported object families rather than silently dropping or serializing sequences. |

#### Issues That Must Not Gate File-Migration Slice 0
...
| Slice 2: snapshot/schema input planning | partial: #259, #172/#253/#254 negative cases | Create parent issue; link migration-adjacent issues as children or blockers. Depends on Slice 0 `gen/parser` hardening (the [Adapt table](#adapt) lists `gen/parser/` as Slice 0 + Slice 2). |
...
These are migration-kit-specific labels that supplement the project-wide scheme in [P2: Apply Labels/Project Fields Consistently](#p2-apply-labelsproject-fields-consistently). Apply them after issue rewriting:
...
The "P1 item" column cross-walks each workstream row to its numbered P1 entry. This makes the [Exit Criteria](#exit-criteria-before-slice-0) check — "one parent issue exists for every project-wide workstream" — mechanically decidable.

| Workstream | P1 item | Source specs | Current posture | Planning action |
| --- | --- | --- | --- | --- |
| Schema DSL and type system | P1 #1 | `schema.md`, `types.md`, `dialects.md` | Partial implementation with known parity gaps. | Create parent issue for schema/type parity and strict file-migration schema input. |
| Query builder and relations | P1 #2 | `query-builder.md`, `relations.md`, `dialects.md` | Substantial implementation with remaining gaps and dialect-gating bugs. | Create parent issue/milestone for query parity cleanup. |
| Codegen | P1 #3 | `codegen.md`, `types.md` | Implemented but narrower than target. | Create parent issue for codegen type mapping, managed output, metadata, and nullable/JSON decisions. |
| Drivers, transactions, prepared statements | P1 #4 | `transactions.md`, `query-builder.md` | Partial implementation; pgx stronger than database/sql. | Create parent issue for transaction/prepared driver parity. |
| Dialects | P1 #5 | `dialects.md` | Useful current matrix; some spec/interface drift. | Create parent issue for dialect feature matrix and capability gating; keep synced with query/schema/driver work. |
| Migration kit / file migrations | P1 #6 | `kit.md`, `file-migrations-*.md` | Specs detailed; target implementation largely absent; legacy code conflicts exist. | Follow slices 0-8 after backlog normalization. |
| Pull / introspection | P1 #7 | `pull.md`, `file-migrations-snapshot-fields.md`, `schema.md`, `codegen.md` | Introspection exists, source generation/pull workflow absent. | Create parent issue for the pull/introspection workflow; implement after core artifact/history/migrate path, per Slice 7. |
| Docs/release/policy | P1 #8 | `README.md`, `docs/spec/README.md`, `overview.md` | Good pre-release posture; issue labels/milestones need normalization. | Keep docs/spec/current-state pages synchronized with implementation. |
...
2. Fix or decide near-term schema/codegen correctness issues (#234, #235, #236, #259) that would poison generated schema metadata. See [Schema / Types / Codegen](#schema--types--codegen) area note for triage context.
3. Start file-migration Slice 0 only after migration issues are rewritten/superseded and slice parent issues exist.
4. Build file-migration slices 0-8 in order, gated by step 1 and step 3. See [Slice Mapping Summary](#slice-mapping-summary) for code involvement and [Per-Slice Parent Issues](#per-slice-parent-issues) for issue mapping.
5. Continue non-blocking query/driver/schema parity work in parallel when it has clear specs and disjoint code paths.

### File-Migration Slice Reference

For normative slice definitions see [file-migrations-implementation-sequence.md](./spec/file-migrations-implementation-sequence.md). Slice-keyed tables in this document live at [Slice Mapping Summary](#slice-mapping-summary) (code involvement) and [Per-Slice Parent Issues](#per-slice-parent-issues) (issue mapping).
...
Code-area tracking issues for migration-kit follow-up work. Each must hang off the relevant slice or workstream parent in [P1](#p1-project-wide-parent-issues-canonical) or [Per-Slice Parent Issues](#per-slice-parent-issues). Before Slice 0 implementation starts, create or update tracking issues for:

1. `Quarantine legacy direct-sync push helpers`
   - paths: `kit/apply.go`, `kit/migrate_mysql.go` (push helpers only), `kit/migrate_sqlite.go` (push helpers only), direct-sync docs/examples
   - symbols to remove during cleanup: `Push`, `DryRun` (PostgreSQL, in `kit/apply.go`); `PushMySQL`, `DryRunMySQL` (in `kit/migrate_mysql.go`); `PushSQLite`, `DryRunSQLite` (in `kit/migrate_sqlite.go`). All six are direct-sync helpers that introspect a live DB and apply generated DDL outside the file-migration workflow.
...
2. `Quarantine legacy live-diff migrate/status helpers`
   ...
   - symbols to remove during cleanup: `Migrate`, `MigrateMySQL`, `MigrateSQLite`, `Status`, `StatusMySQL`, `StatusSQLite`, `LoadHistory`, `ChecksumSQL` (the `Status*` helpers must be removed alongside the `Migrate*` paths so no dangling status entry points remain; `LoadHistory` and `ChecksumSQL` reference the legacy `_grizzle_migrations` history table and must not survive into target `migrate`)
...
A holistic backlog is ready for Slice 0 implementation only when every box below is checked. Each item is intended to be checkable mechanically against GitHub state plus this document.
